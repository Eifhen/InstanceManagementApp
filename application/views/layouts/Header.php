<!DOCTYPE html>
<html lang="en">
<head>

	<meta charset="UTF-8">
	<title>Database Management</title>



	<script type ="text/javascript" src="<?= base_url('jquery/jquery-3.4.1.js');?>"></script> 
	<script type ="text/javascript" src="<?= base_url('popper/popper.min.js');?>"></script>
	<script type ="text/javascript" src="<?= base_url('bootstrap/js/bootstrap.js');?>"></script>
	<script type ="text/javascript" src="<?= base_url('bootstrap-select-1.13.9/dist/js/bootstrap-select.js');?>"></script>
	<script type ="text/javascript" src="<?= base_url('sources/functionManagement.js');?>"></script>
	<script type ="text/javascript" src="<?= base_url('sources/functionInstance.js');?>"></script>
	<script type ="text/javascript" src="<?= base_url('sources/functionQuery.js');?>"></script>
	<script> var GlobalUrl = "<?php echo base_url(); ?>" </script>

	<link rel="StyleSheet" type="text/css" href="<?= base_url('bootstrap/css/bootstrap.css');?>"/>
	<link rel="stylesheet" type="text/css" 
	href="<?= base_url('bootstrap-select-1.13.9/dist/css/bootstrap-select.css');?>"/>
	<link rel="StyleSheet" type="text/css" href="<?= base_url('sources/estilos.css');?>"/>


	<script type="text/javascript" 
	src="<?=base_url('DataTable/DataTables-1.10.20/js/jquery.dataTables.js');?>"></script>

	<script type="text/javascript" 
	src="<?=base_url('DataTable/DataTables-1.10.20/js/dataTables.bootstrap4.min.js');?>"></script>

	<link rel="stylesheet" type="text/css" 
	href="<?=base_url('DataTable/DataTables-1.10.20/css/dataTables.bootstrap4.min.css');?>"/>


</head>
<body class="bg-light"> 
